...
def http_get_json(url: str, params: dict, timeout: int = 20) -> dict:
    r = requests.get(url, params=params, timeout=timeout, headers={"User-Agent": "KayakCompass/1.0.1"})
    r.raise_for_status()
    return r.json()
...
st.caption(f"Version {APP_VERSION}. Instant kayak safety rating based on wind.")
...
place = reverse_geocode_name(lat,lon) or "Your location"
...
wdir = safe_float_list(hourly["wind_direction_10m"])
times = hourly["time"]

worst_i = max(range(len(times)), key=lambda i: wind[i]+gust[i])
status = compute_kayak_rating(wind[worst_i], gust[worst_i], big_water)
...
st.markdown(f"**{place}**  \nWorst hour: {int(wind[worst_i])} mph wind, {int(gust[worst_i])} mph gusts")
...
row1 = st.columns(2)
row1[0].metric("Max wind", f"{int(daily['wind_speed_10m_max'][d_idx])} mph")
row1[1].metric("Max gust", f"{int(daily['wind_gusts_10m_max'][d_idx])} mph")

row2 = st.columns(2)
row2[0].metric("Temp", f"{int(daily['temperature_2m_max'][d_idx])}/{int(daily['temperature_2m_min'][d_idx])} F")
row2[1].metric("Rain", f"{daily['precipitation_probability_max'][d_idx]}%")

# Next hours
st.subheader("Next hours")
rows=[]
for i in range(min(10,len(times))):
    rows.append({
        ...
        "Rating": compute_kayak_rating(wind[i],gust[i],big_water)
    })
